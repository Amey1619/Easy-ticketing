...
export class TicketRoute implements Routes {
  ...
  private initializeRoutes() {
    ...
    this.router.get(
      ...
      AuthMiddleware(["admin", "support", "user"]),
      this.ticket.getCommentsById
    );
    this.router.get(
      `${this.path}/history/:id`,
      AuthMiddleware(["admin", "support", "user"]),
      this.ticket.getTicketLogs
    );
    this.router.put(
      `${this.path}/reassign/:id`,
      AuthMiddleware(["admin", "support"]),
      this.ticket.reassignTicket
    );
    this.router.put(
      `${this.path}/resolve/:id`,
      AuthMiddleware(["admin", "support"]),
      this.ticket.resolveTicket
    );
    this.router.post(
      `${this.path}/comments/:id`,
      AuthMiddleware(["admin", "support", "user"]),
