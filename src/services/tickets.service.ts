...
export class TicketsService {
  public async findAllTickets(
    ...
    const sorter: Record<string, any> = {};
    let tickets = [];
  ...
  public async newTicket(ticketData: Ticket): Promise<Ticket> {
    ...
    ticketData.history = [new Types.ObjectId(logData._id)];
  ...
  public async closeTicket(ticketId: string, user: User): Promise<Ticket> {
    ...
    if (!closedTicket) {
      throw new HttpException(
        404,
        `Ticket with the id ${ticketId} not found or Unauthorized Agent`
      );
    }
  ...
  public async createComment(
    ...
    ticket.comments = [new Types.ObjectId(comment._id)];
    ticket.history = [new Types.ObjectId(logData._id)];
    await ticket.save();
    const ticketNew = await ticket.populate([
      "comments",
      { path: "createdBy", select: "email" },
      { path: "assignedAgent", select: "email" },
    ]);
  ...
  public async findAndDeleteTicket(
    ticketId: string,
    user: User
  ): Promise<Ticket> {
    const ticket = await TicketModel.findById(ticketId);
